...
public class Orbot extends SherlockActivity implements TorConstants,
	...
	private ImageProgressView imgStatus = null; // the main touchable image for
												// activating Orbot
	...
	private void appendLogTextAndScroll(String text) {
		if (mTxtOrbotLog != null) {
			...
			if (layout != null) {
				int scrollDelta = layout.getLineBottom(mTxtOrbotLog
						.getLineCount() - 1)
						- mTxtOrbotLog.getScrollY()
						- mTxtOrbotLog.getHeight();
				if (scrollDelta > 0)
	...
	private void showAbout() {
		...
		new AlertDialog.Builder(this)
				.setTitle(getString(R.string.button_about)).setView(view)
				.show();
	}
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		...
		if (item.getItemId() == R.id.menu_start) {
			...
			startWizard();
		} else if (item.getItemId() == R.id.menu_verify_browser) {
	...
	private void doExit() {
		try {
			...
			// one of the confusing things about all of this code is the
			// multiple
			// places where things like "stopTor" are called, both in the
			// Activity and the Service
			// not something to tackle in your first iteration, but i thin we
			// can talk about fixing
			// terminology but also making sure there are clear distinctions in
			// control
			stopTor();

			// perhaps this should be referenced as INTENT_TOR_SERVICE as in
			// startService
	...
	private void handleIntents() {
		...
		if (action.equals("org.torproject.android.REQUEST_HS_PORT")) {
			...
			String requestMsg = getString(R.string.hidden_service_request,
					hsPort);
			AlertDialog.Builder builder = new AlertDialog.Builder(this);
			builder.setMessage(requestMsg)
	...
	private void showAlert(String title, String msg, boolean button) {
		...
		if (button) {
			aDialog = new AlertDialog.Builder(Orbot.this)
					.setIcon(R.drawable.icon).setTitle(title).setMessage(msg)
	...
	/*
	 * Set the state of the running/not running graphic and label this all needs
	 * to be looked at w/ the shift to progressDialog
	 */
	public void updateStatus(String torServiceMsg) {
		try {
			// if the serivce is bound, query it for the curren status value
			// (int)
			if (mService != null)
			...
			if (imgStatus != null) {
				...
				if (torStatus == TorServiceConstants.STATUS_ON) {
					...
					boolean showFirstTime = mPrefs.getBoolean(
							"connect_first_time", true);
	...
	// guess what? this start's Tor! actually no it just requests via the local
	// ITorService to the remote TorService instance
	// to start Tor
	private void startTor() throws RemoteException {
		...
		Message msg = mHandler
				.obtainMessage(TorServiceConstants.ENABLE_TOR_MSG);
	...
	private void stopTor() throws RemoteException {
		if (mService != null) {
			...
			Message msg = mHandler
					.obtainMessage(TorServiceConstants.DISABLE_TOR_MSG);
			mHandler.sendMessage(msg);
	...
	public boolean onLongClick(View view) {
		...
		if (!mDrawerOpen) {
			try {
				...
				if (mService != null
						&& mService.getStatus() == TorServiceConstants.STATUS_OFF) {
	 ...
	 * to the service, as it should send it as it changes or when we bind/unbind
	 * to it from this activity
	 */
	private ITorServiceCallback mCallback = new ITorServiceCallback.Stub() {
		...
		// receive a new string vaule end-user displayable message from the
		// ITorService
		public void statusChanged(String value) {

			// pass it off to the progressDialog
			Message msg = mHandler
					.obtainMessage(TorServiceConstants.STATUS_MSG);
		...
		@Override
		public void updateBandwidth(long upload, long download, long written,
				long read) {
	...
	// this is what takes messages or values from the callback threads or other
	// non-mainUI threads
	...
	private Handler mHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case TorServiceConstants.LOG_MSG:
				...
				String torServiceMsg = (String) msg.getData().getString(
						HANDLER_TOR_MSG);

				if (lastServiceMsg == null
						|| !lastServiceMsg.equals(torServiceMsg)) {
			...
			case TorServiceConstants.MESSAGE_TRAFFIC_COUNT:
				...
				DataCount datacount = new DataCount(data.getLong("upload"),
						data.getLong("download"));
	...
	private void bindService() {
		...
		bindService(new Intent(ITorService.class.getName()), mConnection,
	...
	/*
	 * private void createProgressDialog (String msg) { if (progressDialog !=
	 * null && progressDialog.isShowing()) { progressDialog.setMessage(msg); }
	 * else { progressDialog = ProgressDialog.show(Orbot.this, "", msg);
	...
	public void spinOrbot(float direction) {
		try {
			...
			// ROTATE_TO*direction, Animation.RELATIVE_TO_SELF, 0.5f,
			// Animation.RELATIVE_TO_SELF, 0.5f);
			Rotate3dAnimation rotation = new Rotate3dAnimation(ROTATE_FROM,
					ROTATE_TO * direction, imgStatus.getWidth() / 2f,
					imgStatus.getWidth() / 2f, 20f, false);
